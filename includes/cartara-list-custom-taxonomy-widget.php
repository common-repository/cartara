...
add_action( 'widgets_init', 'init_cartara_taxonomy' );
function init_cartara_taxonomy() { return register_widget('cartara_taxonomy'); }
class cartara_taxonomy extends WP_Widget {
	/** constructor */
	function __construct() {
		parent::__construct( 'cartara_taxonomy', $name = 'List Product Catgories' );
	}
	/**
	* This is the Widget
	**/
	function widget( $args, $instance ) {
		...
		if($dropdown){
			...
			if( in_array( $tax, array( 'category', 'post_tag', 'post_format' ) ) )
				...
				$walker = new ctwidget_Taxonomy_Dropdown_Walker();
			$args = array(
	...
	**/
	function form( $instance ) {
		//for showing/hiding advanced options; wordpress moves this script to where it needs to go
		  ...
		  // instance exist? if not set defaults
		    if ( $instance ) {
				...
				$childof  = '';
				$this_taxonomy = 'cartara_category';//this will display the category taxonomy, which is used for normal, built-in posts
				$hierarchical = true;
				$showcount = true;
				$empty = false;
				$dropdown = false;
		...
		// The widget form ?>
			...
			<input type="hidden" name="<?php echo $this->get_field_name('taxonomy'); ?>" id="<?php echo $this->get_field_id('taxonomy'); ?>" value="cartara_category"/>
...
class ctwidget_Taxonomy_Dropdown_Walker extends Walker {
